...
-- -- def BooleanFunc {n : Nat} {α : Type u } [Boolean α]  := (Vector (Boolean α)  n) → (Boolean α)
...
-- -- -- def χ {n : Nat} [Boolean α] (S : Vector Bool n) : BooleanFunc := fun (x : ) => x ⬝ S

-- -- def function_inner_product {n : Nat} (x y : BooleanFunc n) := 
-- --   1 / (Rat.pow 2 n) * (∑ i in (Finset.range (2^n)), (x i) * (y i))

-- -- def fourier_coefficient {n : Nat} (x : BooleanFunc n) (S : Vector (Boolean α) n) := 
-- --   function_inner_product x (χ S)

-- -- def fourier_function {n : Nat} (f : BooleanFunc n) := 
-- --   fun (S : Vector (Boolean α) n) => fourier_coefficient f S * (χ S)

-- -- theorem Plancherel_Theorem {n : Nat} (f g: BooleanFunc n) : function_inner_prod f g = function_inner_prod (fourier_function f) (fourier_function g) 
-- --   := sorry
